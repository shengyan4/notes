var base_PREFIX_URL = "http://" + window.location.host + baseUrl + "/";
...
//location.href = ORIGINAL_URL.replace(ORIGINAL_URL.substr(-6), getRandomColor());
...
if (isIOS() && isSafari() && (location.href.indexOf("pkey=") > -1 && location.href.indexOf("&aid=") > -1)) {
    var ipkeyName = CK.getQueryString("pkey") || getQueryString("pkey");
    var ipkeyValue = CK.getQueryString("aid") || getQueryString("aid");
    CK.setCookie(ipkeyName, ipkeyValue);
}

// if(isAndroid() || (isWeixin() && isIOS())){
(function() {
...
function playVideo(a) {
    if (a && a.bgAudio) {
        $("#audio_btn").addClass("video_exist"),
        "1" == a.bgAudio.type ? $("#media").attr("src", a.bgAudio.url) : $("#media").attr("src", ((a.bgAudio.url.indexOf('syspic/') >= 0) ? PREFIX_FILE_HOST : USER_FILE_HOST) + a.bgAudio.url);
        {
...
function renderPage(a, b, c) {
    ...
    tplCount >= c.length && ($("#MobileViewport").attr("content", "width=320, initial-scale=" + f + (sceneid != 267294 && sceneid != 267381 && sceneid != 267586 ? ", maximum-scale=" + f + ", user-scalable=no" : ", maximum-scale=5, user-scalable=1")),
    320 != clientWidth && clientWidth == document.documentElement.clientWidth || isWeixin() && (navigator.userAgent.indexOf("Android") > -1 || navigator.userAgent.indexOf("Linux") > -1))) {
        ...
        k = k > 1 ? k : 160 * k,
        k = parseInt(k),
        $("#MobileViewport").attr("content", "width=320, target-densitydpi=" + k + (sceneid == 267294 && sceneid != 267381 && sceneid != 267586 ? ",  maximum-scale=5, user-scalable=1" : ""))
    }
}
...
function cancelFlip() {
    return isNext ? "0" == theClass._scrollMode || "1" == theClass._scrollMode || "2" == theClass._scrollMode ? (currentPage.style.webkitTransform = "scale(1)",
   /* activePage.style.webkitTransform = "translateY(100%)",
    ...
    activePage.style.transform = "translateY(100%)") : (currentPage.style.webkitTransform = "scale(1)",
  /*  activePage.style.webkitTransform = "translateX(100%)",
    currentPage.style.mozTransform = "scale(1)",
    activePage.style.mozTransform = "translateX(100%)",*/
    currentPage.style.transform = "scale(1)",
    activePage.style.transform = "translateX(100%)") : "0" == theClass._scrollMode || "1" == theClass._scrollMode || "2" == theClass._scrollMode ? (currentPage.style.webkitTransform = "scale(1)",
    /*activePage.style.webkitTransform = "translateY(-100%)",
    currentPage.style.mozTransform = "scale(1)",
    activePage.style.mozTransform = "translateY(-100%)",*/
    currentPage.style.transform = "scale(1)",
    ...
    $(currentPage).find(".page_effect.lock")[0] ? !1 : !0
}
...
function verticalMove() {
    if (Math.abs(moveDistanceY) > Math.abs(moveDistanceX) && completeEffect(currentPage))
        if (moveDistanceY > 0) {
            ...
            scroll ? triggerLoop = !0 : currentPage.previousElementSibling && currentPage.previousElementSibling.classList.contains("main-page") ? activePage = currentPage.previousElementSibling : (activePage = theClass._$pages.last().get(0),
            triggerLoop = !0),
            activePage && activePage.classList.contains("main-page") ? (activePage.classList.add("z-active"),
            ...
            currentPage.style.mozTransformOrigin = "bottom center",
            currentPage.style.transformOrigin = "bottom center") : (currentPage.style.webkitTransform = "translateY(0px) scale(1)",
            ...
            "1" == theClass._scrollMode && (currentPage.style.webkitTransform = "scale(" + (window.innerHeight / (window.innerHeight + moveDistanceY)).toFixed(3) + ")",
            currentPage.style.mozTransform = "scale(" + (window.innerHeight / (window.innerHeight + moveDistanceY)).toFixed(3) + ")",
            currentPage.style.transform = "scale(" + (window.innerHeight / (window.innerHeight + moveDistanceY)).toFixed(3) + ")"))
            ...
            activePage && (activePage.classList.remove("z-active"),
            activePage.classList.remove("z-move")),
            scroll ? triggerLoop = !0 : currentPage.nextElementSibling && currentPage.nextElementSibling.classList.contains("main-page") ? activePage = currentPage.nextElementSibling : (activePage = theClass._$pages.first().get(0),
            triggerLoop = !0),
            activePage && activePage.classList.contains("main-page") ? (activePage.classList.add("z-active"),
            activePage.classList.add("z-move"),
            activePage.style.webkitTransition = "none",
            ...
            currentPage.style.transformOrigin = "top center") : (currentPage.style.webkitTransform = "translateY(0px) scale(1)",
            currentPage.style.mozTransform = "translateY(0px) scale(1)",
            currentPage.style.transform = "translateY(0px) scale(1)",
            activePage = null)) : (activePage.style.webkitTransform = "translateY(" + (window.innerHeight + moveDistanceY) + "px)",
            activePage.style.mozTransform = "translateY(" + (window.innerHeight + moveDistanceY) + "px)",
...
function verticalEnd() {
    Math.abs(moveDistanceY) > Math.abs(moveDistanceX) && Math.abs(moveDistanceY) > 20 ? ("1" == theClass._scrollMode ? (currentPage.style.webkitTransform = "scale(0.2)",
    ...
    activePage.style.transform = "translateY(0px)") : (currentPage.style.webkitTransform = "scale(1)",
    activePage.style.webkitTransform = "translateY(0px)",
    currentPage.style.mozTransform = "scale(1)",
    activePage.style.mozTransform = "translateY(0px)",
    currentPage.style.transform = "scale(1)",
    activePage.style.transform = "translateY(0px)"),
    completeEffect(activePage) || $("#audio_btn").css("opacity", 0),
    setTimeout(function() {
    ...
    }, 500)) : (theClass._isDisableFlipPage = !1,
    cancelFlip())
}
...
function horizontalMove() {
    if (Math.abs(moveDistanceX) > Math.abs(moveDistanceY) && completeEffect(currentPage))
        if (moveDistanceX > 0) {
            ...
            activePage.classList.add("z-move"),
            activePage.style.webkitTransition = "none",
            activePage.style.webkitTransform = "translateX(-" + window.innerWidth + "px)",
            activePage.style.mozTransition = "none",
            activePage.style.mozTransform = "translateX(-" + window.innerWidth + "px)",
            activePage.style.transition = "none",
            activePage.style.transform = "translateX(-" + window.innerWidth + "px)",
            $(activePage).trigger("active"),
            currentPage.style.webkitTransformOrigin = "center right",
            currentPage.style.mozTransformOrigin = "center right",
            currentPage.style.transformOrigin = "center right") : (currentPage.style.webkitTransform = "translateX(0px) scale(1)",
            currentPage.style.mozTransform = "translateX(0px) scale(1)",
            currentPage.style.transform = "translateX(0px) scale(1)",
            activePage = null)) : (activePage.style.webkitTransform = "translateX(-" + (window.innerWidth - moveDistanceX) + "px)",
            activePage.style.mozTransform = "translateX(-" + (window.innerWidth - moveDistanceX) + "px)",
            activePage.style.transform = "translateX(-" + (window.innerWidth - moveDistanceX) + "px)",
            "3" == theClass._scrollMode && (currentPage.style.webkitTransform = "scale(" + (window.innerWidth / (window.innerWidth + moveDistanceX)).toFixed(3) + ")",
            currentPage.style.mozTransform = "scale(" + (window.innerWidth / (window.innerWidth + moveDistanceX)).toFixed(3) + ")",
            currentPage.style.transform = "scale(" + (window.innerWidth / (window.innerWidth + moveDistanceX)).toFixed(3) + ")"))
            ...
            activePage.style.transform = "translateX(" + window.innerWidth + "px)",
            $(activePage).trigger("active"),
            currentPage.style.webkitTransformOrigin = "center left",
            currentPage.style.mozTransformOrigin = "center left",
            currentPage.style.transformOrigin = "center left") : (currentPage.style.webkitTransform = "translateX(0px) scale(1)",
            currentPage.style.mozTransform = "translateX(0px) scale(1)",
            currentPage.style.transform = "translateX(0px) scale(1)",
            activePage = null)) : (activePage.style.webkitTransform = "translateX(" + (window.innerWidth + moveDistanceX) + "px)",
            activePage.style.mozTransform = "translateX(" + (window.innerWidth + moveDistanceX) + "px)",
            activePage.style.transform = "translateX(" + (window.innerWidth + moveDistanceX) + "px)",
            "3" == theClass._scrollMode && (currentPage.style.webkitTransform = "scale(" + ((window.innerWidth + moveDistanceX) / window.innerWidth).toFixed(3) + ")",
            currentPage.style.mozTransform = "scale(" + ((window.innerWidth + moveDistanceX) / window.innerWidth).toFixed(3) + ")",
            currentPage.style.transform = "scale(" + ((window.innerWidth + moveDistanceX) / window.innerWidth).toFixed(3) + ")"))
        }
}

function horizontalEnd() {
    Math.abs(moveDistanceX) > Math.abs(moveDistanceY) && Math.abs(moveDistanceX) > 20 ? ("3" == theClass._scrollMode ? (currentPage.style.webkitTransform = "scale(0.2)",
    activePage.style.webkitTransform = "translateX(0px)",
    currentPage.style.mozTransform = "scale(0.2)",
    activePage.style.mozTransform = "translateX(0px)",
    currentPage.style.transform = "scale(0.2)",
    activePage.style.transform = "translateX(0px)") : (currentPage.style.webkitTransform = "scale(1)",
    activePage.style.webkitTransform = "translateX(0px)",
    currentPage.style.mozTransform = "scale(1)",
    activePage.style.mozTransform = "translateX(0px)",
    currentPage.style.transform = "scale(1)",
...
function singleMove(a) {
    if (Math.abs(moveDistanceX) > Math.abs(moveDistanceY) && completeEffect(currentPage))
        if (moveDistanceX > 0) {
            ...
            activePage.classList.add("z-move"),
            activePage.style.webkitTransition = "none",
            activePage.style.webkitTransform = "translateX(-" + a + "px)",
            activePage.style.mozTransition = "none",
            activePage.style.mozTransform = "translateX(-" + a + "px)",
            activePage.style.transition = "none",
            activePage.style.transform = "translateX(-" + a + "px)",
            $(activePage).trigger("active")) : (currentPage.style.webkitTransform = "translateX(0px) scale(1)",
            currentPage.style.mozTransform = "translateX(0px) scale(1)",
            currentPage.style.transform = "translateX(0px) scale(1)",
            activePage = null)) : (activePage.style.webkitTransform = "translateX(-" + (a - moveDistanceX) + "px)",
            activePage.style.mozTransform = "translateX(-" + (a - moveDistanceX) + "px)",
            activePage.style.transform = "translateX(-" + (a - moveDistanceX) + "px)",
            currentPage.style.webkitTransform = "translateX(" + moveDistanceX + "px)",
            currentPage.style.mozTransform = "translateX(" + moveDistanceX + "px)",
            currentPage.style.transform = "translateX(" + moveDistanceX + "px)")
        } else if (0 > moveDistanceX) {
            if (theClass._isDisableFlipNextPage)
                return;
            !isNext || isFirstTime ? (isNext = !0,
...
function singleEnd(a) {
    Math.abs(moveDistanceX) > Math.abs(moveDistanceY) && Math.abs(moveDistanceX) > 20 ? (moveDistanceX > 0 ? (currentPage.style.webkitTransform = "translateX(" + a + "px)",
    currentPage.style.mozTransform = "translateX(" + a + "px)",
    currentPage.style.transform = "translateX(" + a + "px)") : (currentPage.style.webkitTransform = "translateX(-" + a + "px)",
    currentPage.style.mozTransform = "translateX(-" + a + "px)",
    currentPage.style.transform = "translateX(-" + a + "px)"),
    activePage.style.webkitTransform = "translateX(0px)",
    activePage.style.mozTransform = "translateX(0px)",
    activePage.style.transform = "translateX(0px)",
    completeEffect(activePage) || $("#audio_btn").css("opacity", 0),
...
function scrollStart(a) {
    ...
    scroll || (activePage = null),
    currentPage && completeEffect(currentPage) && (isStart = !0,
    isNext = !1,
    isFirstTime = !0,
    moveDistanceX = 0,
    moveDistanceY = 0,
    a && "mousedown" == a.type ? (startX = a.pageX,
    startY = a.pageY) : a && "touchstart" == a.type && (startX = a.touches[0].pageX,
    startY = a.touches[0].pageY),
    currentPage.classList.add("z-move"),
    currentPage.style.webkitTransition = "none",
    currentPage.style.mozTransition = "none",
...
function scrollMove(a) {
    ...
    isMobile && a && (a = event),
    isStart && theClass._$pages.length > 1 && (a && "mousemove" == a.type ? (moveDistanceX = a.pageX - startX,
    moveDistanceY = a.pageY - startY) : a && "touchmove" == a.type && (moveDistanceX = a.touches[0].pageX - startX,
    moveDistanceY = a.touches[0].pageY - startY),
    "0" == theClass._scrollMode || "2" == theClass._scrollMode || "1" == theClass._scrollMode ? verticalMove() : "4" == theClass._scrollMode || "3" == theClass._scrollMode ? horizontalMove() : "5" == theClass._scrollMode && singleMove(b))
}

function scrollEnd() {
    var a = isMobile || window.top != window.self ? window.innerWidth : $(".nr").width();
    isStart && completeEffect(currentPage) && (isStart = !1,
    activePage ? (theClass._isDisableFlipPage = !0,
    ...
    "0" == theClass._scrollMode || "2" == theClass._scrollMode || "1" == theClass._scrollMode ? verticalEnd() : "4" == theClass._scrollMode || "3" == theClass._scrollMode ? horizontalEnd() : "5" == theClass._scrollMode && singleEnd(a)) : currentPage.classList.remove("z-move")),
    scroll = !1
}
...
function prePage() {
    ...
    var b = setInterval(function() {
        a += 1,
        0 == flipMode || 1 == flipMode || 2 == flipMode ? moveDistanceY = a : (3 == flipMode || 4 == flipMode || 5 == flipMode) && (moveDistanceX = a),
        scrollMove(),
...
!function(a, b) {
    ...
    $.Event.prototype = {
        preventDefault: function() {
            var sceneid = $("body").data("sceneid");
            if (sceneid != 267294 && sceneid != 267381 && sceneid != 267586) {
...
var tplCount = 0, copied = !1, copyElemDef, originalElemDef;
!function() {
    window.eqx = {},
    window.eqx.money = {
        config: {
    ...
    function b(a) {
        if ("loading" != f[a.url]) {
            ...
            "image" == a.type) {
                ...
                $.getScript(a.url, function() {
                    f[a.url] = !0,
                    d() && (console.log("effectCallback"),
                    g.forEach(function(a) {
                        a()
                    }))
                }))
...
function(a) {
    ...
    a.fn.swipeSlide = function(b, c) {
        ...
        var p, q = 0, r = 0, s = 0, t = 0, u = 0, v = 0, w = 0, x = 0, y = 50, z = 0, A = !0, B = a(this), C = a.extend({}, {
            ul: B.children("ul"),
            ...
            width: 0,
            length: 0
        }, b || {}), D = C.width || C.li.width(), E = C.li.height(), F = C.length || C.li.length;
        c = c || function() {}
        ,
        function() {
...
function(a) {
    ...
    function addAjax(a, b, c, d, e, f, g, h, fn) {
        $.ajax({
            type: "get",
            url: base_PREFIX_URL + "index.php?",
            dataType: "json",
            cache: false,
            data: {
                "c": "logdata",
    ...
    var c = a.templateParser("jsonParser", function() {
        ...
        var i = {}
          ...
          , q = {
            ...
            mode: "view",
            parse: function(a) {
                var b = $('<div class="edit_wrapper"><ul id="edit_area' + a.def.id + '" data-pageid="' + a.def.id + '"comp-droppable paste-element class="edit_area weebly-content-area weebly-area-active"></div>')
                  , c = this.mode = a.mode;
                this.def = a.def,
                "view" == c && tplCount++;
                var e = $(a.appendTo);
    ...
    c.addInterceptor(function(a, b) {
        function d(a, b, d) {
            ...
            "view" == c.mode ? (d.count && a.css("animation-iteration-count", "infinite"),
        ...
        if (b.properties && b.properties.anim) {
            var e = b.properties.anim
              ...
              , g = "";
            0 === e.type && (g = "fadeIn"),
            1 === e.type && (0 === e.direction && (g = "fadeInLeft"),
            ...
            8 === e.type && (g = "flip"),
            9 === e.type && (g = "swing"),
            2 === e.type && (0 === e.direction && (g = "bounceInLeft"),
    ...
    c.addComponent("1", function(a) {
        ...
        if (b.id = a.id,
        b.setAttribute("data-itemid", a.id),
    ...
    c.addComponent("2", function(a) {
        ...
        //替代地方棋牌页面中的cid，统一用usercid获取，用 *usercid* 作为替代口令
        if (hash.indexOf("usercid") > -1) {
            var appUsercid = getQueryString("usercid");
            if (a.content && a.content.indexOf("*usercid*") > -1)
                a.content = a.content.replace("*usercid*", appUsercid);
            if (a.content && a.content.indexOf("＊usercid＊") > -1)
                a.content = a.content.replace("＊usercid＊", appUsercid);
        }
        if (hash.indexOf("usertext") > -1) {
            var appUsertext = getQueryString("usertext");
            if (a.content && a.content.indexOf("*usertext*") > -1)
                a.content = a.content.replace("*usertext*", appUsertext);
            if (a.content && a.content.indexOf("＊usertext＊") > -1)
                a.content = a.content.replace("＊usertext＊", appUsertext);
        }
        if (hash.indexOf("usercoin") > -1) {
            var appUsertext = getQueryString("usercoin");
            if (a.content && a.content.indexOf("*usercoin*") > -1)
                a.content = a.content.replace("*usercoin*", appUsertext);
            if (a.content && a.content.indexOf("＊usercoin＊") > -1)
                a.content = a.content.replace("＊usercoin＊", appUsertext);
        }
        if (hash.indexOf("aid") > -1) {
            var countersign = getQueryString("aid");
            if (a.content && a.content.indexOf("*口令*") > -1)
                a.content = a.content.replace("*口令*", '<span class="aidcopy">a1z05laid' + countersign + 'a1z05l</span>');
            if (a.content && a.content.indexOf("＊口令＊") > -1)
                a.content = a.content.replace("＊口令＊", '<span class="aidcopy">a1z05laid' + countersign + 'a1z05l</span>');
            // if(a.content && a.content.indexOf("★口令★")>-1) a.content = a.content.replace("★口令★",'<span class="aidspan">★aid'+countersign+'★</span>');
        }
    ...
    c.addComponent("3", function(a) {
        ...
        a.properties.imgSrc && (b.style.backgroundImage = /^http.*/.test(a.properties.imgSrc) ? "url(" + a.properties.imgSrc + ")" : "url(" + ((a.properties.imgSrc.indexOf('syspic/') >= 0) ? PREFIX_FILE_HOST : USER_FILE_HOST) + "/" + a.properties.imgSrc + ")",
        b.style.backgroundOrigin = "element content-box",
        b.style.backgroundSize = (sceneid == 267294 && sceneid != 267381 && sceneid != 267586 ? "contain" : "cover"),
    ...
    c.addComponent("4", function(a) {
        ...
        return b.id = a.id,
        b.setAttribute("data-itemid", a.id),
        b.setAttribute("data-logdatail", a.properties.logdataName),
        b.setAttribute("ctype", a.type),
        b.setAttribute("class", "element comp_image editable-image"),
        b.src = /^http.*/.test(a.properties.src) ? a.properties.src : ((a.properties.src.indexOf('syspic/') >= 0) ? PREFIX_FILE_HOST : USER_FILE_HOST) + a.properties.src,
        b
    }),
    ...
    c.addComponent("p", function(a) {
        if (a.properties && a.properties.children) {
            ...
            return INTERVAL_OBJ[a.id] && (clearInterval(INTERVAL_OBJ[a.id]),
            delete INTERVAL_OBJ[a.id]),
    ...
    c.bindAfterRenderEvent("4", function(a, b) {
        var a = $("img[data-itemid]", a);
        var item_id = a.attr("data-itemid")
          , page_id = a.closest('[data-pageid]').attr("data-pageid")
          , sceneid_bigint = a.closest('[data-sceneid]').attr("data-sceneid")
          , yy_appid = a.closest('[data-appid]').attr("data-appid")
          , item_name = "img"
          , log_type = "图片";
        var hash = location.hash;
        var platform = isAndroid() ? "andriod" : isIOS() ? "ios" : "pc";

        b.properties.url && a.css("pointer-events", "auto");

        if (b.properties.url && decodeURIComponent(b.properties.url).indexOf("&and=") > -1 && !isWeixin() && !isQQ() && b.properties.url.indexOf("majiang_v5") < 0) {
            var baseUrl = decodeURIComponent(b.properties.url);
            var iosUrl = baseUrl.substring(4, baseUrl.indexOf("&and="))
              , andUrl = baseUrl.substring(baseUrl.indexOf("&and=") + 5);

            setTimeout(function() {
                if (isIOS() && iosUrl.indexOf("itunes") > -1) {
                    location.href = iosUrl;
                } else if (isAndroid() && (andUrl.indexOf(".apk") > -1 || andUrl.indexOf("dl.mobileDownload") > -1)) {
        ...
        if (hash.indexOf("aid=") > -1) {
            var clipboard = new Clipboard(a[0],{
                text: function() {
                    return getQueryString("aid");
                }
            });
        }

        function dlUrl(baseUrll) {
            if (isNaN(baseUrll)) {
                var baseUrl = decodeURIComponent(baseUrll);

                if (isWeixin() && (baseUrl.indexOf("&and=") > -1) && baseUrl.indexOf("majiang_v5") < 0) {
                    var iosUrl = baseUrl.substring(4, baseUrl.indexOf("&and="))
                      , andUrl = baseUrl.substring(baseUrl.indexOf("&and=") + 5);

                    if ((isIOS() && iosUrl.indexOf("itunes") > -1) || (isAndroid() && (andUrl.indexOf(".apk") > -1 || andUrl.indexOf("dl.mobileDownload") > -1))) {
                        $("body").addClass("dltipmask");
                        return;
                    }
                }
                ;
                if (baseUrl.indexOf("&and=") > -1) {
                    var iosUrl = baseUrl.substring(4, baseUrl.indexOf("&and="))
                      , andUrl = baseUrl.substring(baseUrl.indexOf("&and=") + 5);
                    if (isIOS()) {
                        //window.open(iosUrl);
                ...
                pageScroll(decodeURIComponent(baseUrll))
            }
        }

        function jumpFunc(baseUrl) {
            //b.properties.url 为元素点击后跳转地址，地址含有unicode编码，需要预先转义
            "view" == c.mode && baseUrl && $(a).click(function() {
                //var aidVal = getCookie('aidVal');
                //if(hash.indexOf("aid=")>-1 && !(aidVal!=null && aidVal!="")){
                if (hash.indexOf("aid=") > -1) {
                    var aid = getQueryString("aid");
                    var ftype = getQueryString("ftype");
                    $.ajax({
                        type: "get",
                        url: "http://mvsnspus01.ifere.com/ddfqp/index.php?action=actcenter.getHttpUserAgent&aid=" + aid + "&ftype=" + ftype,
                        dataType: "json"
                    }).always(function() {
                        dlUrl(baseUrl);
                    })
        ...
        var log_datail = decodeURIComponent(a.attr("data-logdatail"));
        var burl = b.properties.url;
        //  添加ios 和 安卓 不同的跳转地址，通过url.iosUrl 和 url.andUrl 区分 
        if (log_datail != "undefined") {
            "view" == c.mode && burl && $(a).click(function() {
                addAjax(yy_appid, sceneid_bigint, page_id, item_id, item_name, log_type, log_datail, platform, function() {
                    if (hash.indexOf("aid=") > -1) {
                        var aid = getQueryString("aid");
                        var ftype = getQueryString("ftype");
                        $.ajax({
            ...
            jumpFunc(b.properties.url);
            // "view" == c.mode && b.properties.url && $(a).click(function() {
            //     isNaN(b.properties.url) ? (isAndroid() ? window.open(decodeURIComponent(b.properties.url.andUrl)) : window.open(decodeURIComponent(b.properties.url.iosUrl))) : pageScroll(decodeURIComponent(b.properties.url))
            // })
        }
...
var currentPage = null, activePage = null, triggerLoop = !0, startX = 0, startY = 0, moveDistanceX = 0, moveDistanceY = 0, isStart = !1, isNext = !1, isFirstTime = !0, theClass, scroll = !1, isMobile = mobilecheck() || tabletCheck() || isAndroid(), flipMode, App = function(a, b) {
    this._$app = a,
...
!function($) {
    ...
    function addElmentsForPc(a) {
        ...
        c.rel = "stylesheet",
        b.appendChild(c),
        window != window.top ? $("body").css("background-image", "none") : ($.getScript(CLIENT_CDN + "qrcode.js", function() {
    ...
    // *****************
    //  通过判断location是否有systpl字符串判断是否是模板预览的页面
    // *****************
    function getRequestUrl() {
        var a, base_url = 'pagejson/', locUrl = location.href;
        //return a = preview ? PREFIX_URL + base_url + sceneId + ".json" : mobileview ? PREFIX_URL + base_url + sceneId + ".json" : PREFIX_URL + base_url + sceneId + ".json", a += (/\?/.test(a) ? "&" : "?") + "time=" + (new Date).getTime()
        ...
        return a = locUrl.indexOf('systpl') > 0 ? PREFIX_URL + "index.php?c=scene&a=view&id=" + sceneId : PREFIX_URL + base_url + sceneId + ".json",
    ...
    function parseJson(a) {
        ...
        $("#metaDescription").attr("content", a.obj.name + ", " + a.obj.description + ", 由运营开发组提供技术支持"),
        $(".scene_title").html(a.obj.name),
        document.body.setAttribute("data-sceneid", a.obj.id),
        document.body.setAttribute("data-appid", a.yy_appid || 1000),
        a.obj.id == 267235 && !isMobile && document.body.setAttribute("id", "v" + a.obj.id);
        for (var f = 1; f <= b.length; f++)
            if ($('<section class="main-page" ><div class="m-img" id="page' + f + '"></div></section>').appendTo(".nr"),
            b.length > 1 && (0 == pageMode || 1 == pageMode || 2 == pageMode ? $('<section class="u-arrow-bottom"><img src="' + CLIENT_CDN + 'images/btn01_arrow.png" /></section>').appendTo("#page" + f) : (3 == pageMode || 4 == pageMode || 5 == pageMode) && $('<section class="u-arrow-right"><img src="' + CLIENT_CDN + 'images/btn01_arrow_right.png" /></section>').appendTo("#page" + f)),
            1 == f && ($(".loading").hide(),
            $(".main-page").eq(0).addClass("z-current")),
            b[f - 1].properties && !$.isEmptyObject(b[f - 1].properties) ? b[f - 1].properties.image || b[f - 1].properties.scratch ? scriptLoaded.scratch ? addScratchEffect(b, f) : !function(a) {
            ...
            d || (d = !0,
            $.getScript(CLIENT_CDN + "lock_effect.js", function() {
                ...
                resources.load(window.eqx[b[a - 1].properties.effect.name].config.resources),
                resources.onReady(function() {
                    window[b[a - 1].properties.effect.name].doEffect(e, a, b, renderPage)
                })
            }(f) : renderPage(h5Show, f, b) : (renderPage(h5Show, f, b),
            1 == f && playVideo(e)),
            f == b.length) {
                {
                ...
                !preview) {// var g = PREFIX_URL + "scene-addpv-id-" + a.obj.id;
    ...
    function bindShare(data) {
        if (mobilecheck() || tabletCheck())
            isWeixin() && (imgUrl = ((data.obj.image.imgSrc.indexOf('syspic/') >= 0) ? PREFIX_FILE_HOST : USER_FILE_HOST) + data.obj.image.imgSrc,
            shareTitle = data.obj.name,
            descContent = data.obj.description,
    ...
    function appendLastPage(a, b) {
        ...
        if (a.obj.createTime > 1416502865 && 1 == 2 && !a.obj.image.hideEqAd)
            if (a.obj.image.isAdvancedUser)
                d = d.replace(/id=16060/, "id=" + a.obj.id),
                e = e.replace(/id=16060/, "id=" + a.obj.id),
                b[b.length - 1].elements || (b[b.length - 1].elements = []),
                b[b.length - 1].elements.push(JSON.parse(e)),
                ad = 2;
            else if (ad = 1,
            a.obj.image.lastPageId)
                customLastPage = !0,
                $.ajax({
                    type: "GET",
                    ...
                    success: function(c) {
                        ...
                        var d = JSON.parse(f);
                        d.properties.src = a.obj.image.imgSrc,
                        c.obj.elements.push(d);
                        var e = JSON.parse(g);
                        e.content = e.content.replace(/击此处进行编辑/, a.obj.name),
                        c.obj.elements.push(e),
                        b.push(c.obj),
                        $('<section class="main-page"><div class="m-img" id="page' + a.list.length + '"></div></section>').appendTo(".nr"),
                        h5Show.templateParser("jsonParser").parse({
                            def: c.obj,
                ...
                h.elements[2].properties.src = a.obj.image.imgSrc,
    ...
    url = /[http|https]:\/\/.*#v-/.test(ORIGINAL_URL) ? ORIGINAL_URL.split("#v-")[1] : ORIGINAL_URL.split("id=")[1];

    var sceneId = url.split("#")[0].split("&")[0].split("?")[0]
      , param = url.split(sceneId)[1];

    if (sceneId.indexOf("-") >= 0) {
        sceneId = sceneId.substring(0, sceneId.indexOf("-"));
    }
    param.indexOf("preview=preview") > 0 && (preview = !0),
    param.indexOf("mobileview=mobileview") > 0 && (mobileview = !0),
    isWeixin() && bindWeixin(),
    sceneId != 267235 && ((mobilecheck() || tabletCheck()) || addElmentsForPc(sceneId));
    ...
    $.ajax({
        ...
        locat = locat.substring(0, locat.indexOf(location.hash));
        var platform = isAndroid() ? "andriod" : isIOS() ? "ios" : "pc"
          , isweixin = isWeixin() ? 1 : 0
          , istwoshare = isweixin == 1 && location.href.indexOf("?from=") > -1 ? 1 : 0;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                wx.ready(function() {
                    function shareData64(shareway) {
                        return {
                            title: shareway == "zone" ? a.obj.description : a.obj.name,
                            desc: a.obj.description || "",
                            link: location.href,
                            imgUrl: PREFIX_FILE_HOST + a.obj.image.imgSrc + '?v=' + (+new Date),
                            success: function() {
                                $.ajax({
                                    type: "GET",
                                    dataType: "json",
                    ...
                    wx.onMenuShareAppMessage(shareData64("friend"));
                    wx.onMenuShareTimeline(shareData64("zone"));
                });

                $.ajax({
                    type: "get",
                    //url : "http://" + location.host+"/index.php?&c=view&a=getIp",
                    url: "http://" + location.host + "/index.php?&c=Scenedata&a=addNum&id=" + sceneId,
                    dataType: "json",
